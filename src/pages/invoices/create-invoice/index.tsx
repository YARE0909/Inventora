...
const Index = () => {
  ...
  const fetchCustomerData = async (filter?: string) => {
    try {
      ...
      // map over response.data and assign id to value and taxPercentage to label
      const data = response.data.filter((item: Customer) => item.isActive).map((customer: Customer) => ({
        value: customer.id,
        label: customer.name,
      }));
  ...
  return (
    <Layout header={(
      ...
      <div className="w-full flex flex-col items-center">
        <div className="w-fit bg-foreground rounded-md p-4 flex flex-col space-y-3">
          {/* Invoice Details */}
